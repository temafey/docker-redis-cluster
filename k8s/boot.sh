...
INSTANCES=6
IMAGE=temafey/redis-cluster-instance:latest
REPLICAS=1
CLEAN=true
SERVICES=''
...
case $i in
    ...
    --help)
        ...
        echo "-img|--image      Docker image. Default -img=temafey/redis-cluster-instance:latest"
...
kubectl run redis-c --image=temafey/redis-trib --restart=Never --command -- sh -c "echo 'yes' | ./redis-trib.rb create --replicas $REPLICAS $SERVICES"
